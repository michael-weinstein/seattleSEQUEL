...
sub main(){
    ...
    print OUTPUT "lineNumber\tuniLoc\t";  #prints the first two fields in the header
    ...
    LINE: while (my $line = <SSI>){  #loop that goes through file while there is file to go through line by line
        ...
        print OUTPUT $progress."\t".$linehash{chromosome}."\:".$linehash{position}."\t";  #prints the first and second fields of a data row, generates the line number trough the counter and the uniLoc by concatenating together the chromosome and locus fields
    ...
    print SQL "ALTER TABLE $tablename ADD INDEX \(lineNumber\)\, ADD INDEX \(uniLoc\)\, ADD INDEX \(geneList\)\;\n";  #creates an index by locus, gene, and line number
